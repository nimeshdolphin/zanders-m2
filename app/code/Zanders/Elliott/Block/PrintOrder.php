...
class PrintOrder extends \Magento\Framework\View\Element\Template
{
    ...
    public function getTracking($order){
        ...
        foreach($order->Notes as $note){
            ...
            if($viacode == 'fdx'){
                $url = "http://www.fedex.com/Tracking?ascend_header=1&clienttype=dotcom&cntry_code=us&language=english&tracknumbers=".$tnumber;
            }
            if($viacode == 'usps'){
                $url = "http://trkcnfrm1.smi.usps.com/PTSInternetWeb/InterLabelInquiry.do?origTrackNum=".$tnumber;
            }
            if($viacode == 'spee-dee'){
                $url = "http://packages.speedeedelivery.com/packages.asp?tracking=".$tnumber;
            }
