...
class EshowPurchasedItem extends AbstractDb
{
    protected function _construct()
    {
        $this->_init('zanders_show_specials_purchase', 'id');
    }

    /**
     * Get Purchased Items data by sku
     *
     * @param int $showId
     * @return array
     */
    public function getByShowId($showId)
    {
        ...
        $select = $connection->select()
            ->from($this->getConnection()->getTableName('zanders_show_specials_purchase'), '*')
    ...
    public function getIdSkuPairs($showId)
    {
        ...
        $select = $connection->select()
            ->from($this->getConnection()->getTableName('zanders_show_specials_purchase'), ['id','sku'])
            ->where('show_id = :show_id');
        $bind = [':show_id' => (int)$showId];
    ...
    public function deleteBySkus($showId, $skus)
    {
        ...
        return $this->getConnection()->delete($this->getTable('zanders_show_specials_purchase'), $where);
    }
}
