...
class MassDelete extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        foreach ($jobData as $value) {
            $templateId[]=$value['toppromo_id'];
        }
        $parameterData = $this->getRequest()->getParams('toppromo_id');
        $selectedAppsid = $this->getRequest()->getParams('toppromo_id');
        ...
        $collection->addFieldToFilter('toppromo_id', ['in'=>$selectedAppsid]);
        $delete = 0;

        foreach ($collection as $item) {
            $this->deleteById($item->getToppromoId());
            $delete++;
        }
